...
export const fmtNumber = (val: any, pretty: boolean = false, format_: numbro.Format = { thousandSeparated: true, mantissa: 2, negative: "parenthesis", spaceSeparated: true }) => {
  if (typeof val === 'number' && !isNaN(val) && !isNull(val)) {
    const fmt: numbro.Format = { ...{ thousandSeparated: true, mantissa: 2, negative: "parenthesis", spaceSeparated: true }, ...format_ }
    const decDot = Intl.NumberFormat().formatToParts(1.1).find(e => e.type === 'decimal').value
    let _dv = numbro(val).format(fmt)
    if (pretty) {
      const resReg = (decDot === '.')
        ? _dv.match(/(([\d,]+)\.(\d+(?!\.))|([\d,]+))([a-z%\s]*)(\)*)/di)
        : _dv.match(/(([\d.]+),(\d+(?!,))|([\d.]+))([a-z%\s]*)(\)*)/di)

      if (resReg && resReg.indices) {
        if (resReg[2] !== undefined && resReg[3] !== undefined)
          _dv = `${_dv.substring(0, resReg.indices[2][1]) + decDot}<small>${resReg[3]}</small>${resReg[5].toUpperCase()}${resReg[6].toUpperCase()}`
        else if (resReg[4] !== undefined)
          _dv = _dv.substring(0, resReg.indices[4][1]) + resReg[5].toUpperCase() + resReg[6].toUpperCase()
      }
    }
...
export const getCtrType = (ctrType: number, mode: number, opt: { nameOnly?: boolean; bracket?: boolean } | undefined = undefined) => {
  ...
  if (mode === 0) {
    if ([1, 3, 4].includes(ctrType)) {
      return (opt?.bracket ? '(' : '') + (opt?.nameOnly
        ? " CR"
        : (locale.value === 'id' ? ` CR ${ctrType === 3 ? '1 ' : ''}` : ` ${ctrType === 3 ? '1st ' : ''}CR`)) + (opt?.bracket ? ')' : '')
    }
    else if ([2, 5, 6].includes(ctrType)) {
      return (opt?.bracket ? '(' : '') + (opt?.nameOnly
        ? " GS"
...
export const VModelPercent = (model: Ref<number | null | undefined>) => computed({
  get: () => model.value ? model.value / 100.0 : 0,
  set: val => {
    if (!isNaN(+val))
      model.value = val * 100
  },
...
export const defContextMenus: contextMenuType[] = [
  { name: 'row_above' },
  { name: 'row_below' },
  { name: 'remove_row' },
  { name: 'separator' },
...
export const TableContextMenus = (menus: contextMenuType[] = defContextMenus) => {
  const _menus = { items: {} }
        ...
        ? {
          [`${value.name}`]: {
            ...
            callback(key, selection, clickEvent) {
              value.callback(key, selection, clickEvent)
            },
          }
        }
        : {
...
export const defProj = (): ProjectBase => ({
  ...
  multicase: [],
  evaluator: '',
  evaluator_date: dayjs.utc().valueOf(),
})
...
export const defFiskalBase = (): FiskalBase => ({
  ...
  sulfur_revenue_config: 2,
  electricity_revenue_config: 2,
