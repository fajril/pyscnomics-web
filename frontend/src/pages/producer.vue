...
const calcYear = (item: producerConfig) => {
  const ArrY = item.prod_price.reduce((year, prod, i) => {
    return [...year, ...prod.reduce((y, row) => {
      const is_valid = is_number(row.year) && ((is_number(row.sales) && is_number(row.price)) ||
        is_number(row.condensate_sales) && is_number(row.condensate_price) ||
        is_number(row.production))
      return [...y, ...(is_valid ? [row.year] : [])]
    }, [])]
  }, [])
  if (ArrY.length)
...
<template>
  <VCard :title="$t('Lifting')" :subtitle="$t('Configuration')">
    <VCardText>
      <VRow>
        <VCol cols="12" md="4">
                ...
                <VExpansionPanel v-for="(item, index) in dataProd" :key="`prod_${item.Tipe}`" :value="item.Tipe"
                  ...
                  <VExpansionPanelText>
                    ...
                    <AppDateTimePicker :model-value="item.onstream_date" label-placeholder="Onstream Date"
                      ...
                      append-icon="tabler-calendar-check" @click:append="() => calcYear(item)" />
                  </VExpansionPanelText>
                </VExpansionPanel>
              </VExpansionPanels>
            </VCardText>
