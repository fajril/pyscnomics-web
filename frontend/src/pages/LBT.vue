...
const { dataLBT } = storeToRefs(PyscConf)

const { TabelContainer, hotTableRef: refTableLBT, hotInstance, htTblSett: mainSetting, updateData, onGetHintTooltip, onAfterValidate } = useHTtable({
  data: dataLBT.value,
  colHeaders: ["Year", "Associated With", "Cost (MUSD)", "LBT Portion", "Description"],
  columns: [
  ...
  beforeRemoveRow(index, amount, physicalRows) {
    const countData = dataLBT.value.length
      ...
      appStore.showXlsxImport({
        ...
        Callback: (data: any[]) => {
          if (data.length === 0) { appStore.showAlert({ text: 'Data empty.', isalert: true }) }
            ...
            dataLBT.value.splice(0, dataLBT.value.length, ...JSON.parse(JSON.stringify(data)))
          }
        },
      })
    },
...
onAfterValidate.value = (isValid, value, row, prop) => {
  ...
  nextTick(() => {
    try {
      if (ctype === 'numeric' && (typeof dataLBT.value[row][+prop] === 'string')
      && dataLBT.value[row][+prop].includes('=')) {
        dataLBT.value[row][+prop] = +(+value).toPrecision(15)
        updateData(dataLBT.value)
      }
    }
    catch (error) {
...
const optOption = [
  ...
  { type: 'divider' },
  { title: 'LBT Calculator', value: 'calc-lbt', icon: 'tabler-calculator' },
  { type: 'divider' },
  ...
  if (tblDataScr && tblDataScr.length) {
    if (sourceType === 'text') {
      ...
      return {
        ...
        filename: `cost_LBT_${appStore.selectedCase.name}_${rndid}`.replace(/[/\\ #$~&.]/g, ''),
      }
    }
  }
