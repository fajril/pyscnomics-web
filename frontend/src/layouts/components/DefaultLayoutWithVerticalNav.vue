    ...
    <template #navbar="{ toggleVerticalOverlayNavActive }">
      <div class="d-flex h-100 align-center">
        ...
        <div>
          <VSelect
            v-model="curSelCase"
            :class="{ 'd-none': $route.name === 'dashboard' }"
            :items="appStore.caseList"
            item-props
            variant="outlined"
            density="compact"
            :style="{ maxInlineSize: '180px', minInlineSize: '100px' }"
            label="Cases"
          />
          <VTooltip
            activator="parent"
          ...
          >
            <span>{{ appStore.caseList.find(v => v.value === curSelCase)?.title }}</span>
          </VTooltip>
        </div>
        <VIcon
