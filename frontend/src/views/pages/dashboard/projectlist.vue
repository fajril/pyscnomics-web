...
const updateSelImportPath = async (value: string) => {
  ...
  try {
    ...
    if (status !== 200 || result.state !== true)
      throw "Invalid file type or it's an old file"
...
<template>
  <VCard :title="$t('My Cases')" :subtitle="$t('List of ', [$t('case')])" :loading="isLoading ? 'primary' : false">
    <template #prepend>
  ...
  <CardCombine v-if="isCombineDlgVisible" ref="cardCombine"
