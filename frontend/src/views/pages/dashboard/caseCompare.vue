...
const calcData = async () => {
  ...
  if (dataLoaded) {
    ...
    dataChartCompare.value.splice(0, dataChartCompare.value.length, ...
      [CompareConf.value.source, ...CompareConf.value.comp].map((cs, idx) => {
        return [
    ...
    dataBarChartCompare.value.splice(0, dataBarChartCompare.value.length, ...CompareConf.value.comp.map((row, idx) => {
      const _rows =
        [{ value: CompOut[20][idx + 1], percent: ((CompOut[20][idx + 1] - CompOut[20][0]) !== 0.0 ? (math.abs((CompOut[20][idx + 1] - CompOut[20][0]) / CompOut[20][0])) : 0.0) * (CompOut[20][idx + 1] < CompOut[20][0] ? -1 : 1) * 100.0 }, //'NCS'
        ...
        <VToolbar color="primary">
          ...
          <VToolbarTitle>
            <div class="d-flex h-100 align-center">
              ...
              <div class="mx-4 my-2" :style="{ maxInlineSize: '170px' }">
                ...
                <VTooltip activator="parent" open-delay="200" scroll-strategy="close">
                  <span>{{ appStore.caseByID(CompareConf.source)?.name }}</span>
                </VTooltip>
              </div>
              <VIcon size="26" icon="tabler-switch-horizontal" />
              <AppSelect v-model="CompareConf.comp" class="mx-2" item-value="value" item-title="name"
                item-subTitle="desc" :items="targetCases" multiple clearable clear-icon="tabler-x"
          ...
          <VCol cols="12">
            <ColCollapsible :col-ratio="[isLessThanCardBreaklg ? 70 : 60, isLessThanCardBreaklg ? 30 : 40]">
              <template #left="{ collapsible, collapsed }">
                <AppCardActions action-collapsed title="Summary" compact-header :collapsed="collapsible"
              ...
              <template #right="{ collapsible, collapsed }">
                <AppCardActions action-collapsed title="Chart" :collapsed="collapsible"
                  @collapsed="val => collapsed(val)" compact-header style="overflow:visible !important">
                  <VCardText class="px-0 py-0">
                    <AppCardActions title="Radar Chart" action-collapsed compact-header
                      style="overflow:visible !important">
                      <ChartCompare :series="dataChartSeries" :dataChart="dataChartCompare" />
                    </AppCardActions>
                    <AppCardActions title="Bar Chart" action-collapsed compact-header class="mt-2"
                      style="overflow:visible !important">
                      <BarChartCompare :baseData="dataBarChartBase" :series="getSeriesBarChart"
                        :dataChart="getResultBarChart" />
                    </AppCardActions>
                  </VCardText>
                </AppCardActions>
              </template>
