      ...
      Callback: (data: any[]) => {
        if (data.length === 0)
          ...
          const _prodData = data.map(row => {
            const _rowData = _columnKeys.reduce((_rowData, ck, i) => {
              if (ck.indexOf('gsa') !== -1) {
                const kArr = ck.match(/([a-z]+)\.(([a-z]+)([1-9]+))/d)
                if (!_rowData.hasOwnProperty('gsa'))
                  return { ..._rowData, ...{ gsa: { [kArr[2]]: row[i] } } }
                else {
                  _rowData.gsa = { ..._rowData.gsa, ...{ [kArr[2]]: row[i] } }
                  return _rowData
                }
              } else
                return { ..._rowData, ...{ [ck]: row[i] } }
            }, {})
            return _rowData
          })
  ...
  <VRow no-gutters>
    ...
    <VCol cols="12">
      <TableEditor v-model:model-value="dataProd[ProdIndex].prod_price[selProdIndex]" :columns="columnTable"
        colAutoWidth="none" :context-menus="contextMenus" />
    </VCol>
  </VRow>
</template>
