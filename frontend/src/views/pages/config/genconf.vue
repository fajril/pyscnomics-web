...
<template>
  <VRow no-gutters>
    ...
    <VCol cols="12" class="mt-2">
      <span class="mt-4 font-weight-bold text-primary">Delayed / Acceleration</span>
      <AppSelect v-model="dataGConf.delayAccMode"
        :items="[{ title: 'None', value: 0 }, { title: 'delayed', value: 1 }, { title: 'Acceleration', value: 2 }]"
        item-props label-placeholder="Delayed/Acceleration" variant="outlined" class="mt-2" />
      <AppTextField v-model.number="dataGConf.delayAccYear" label-placeholder="Year(s)" class="mt-4 ms-4"
        :rules="[requiredValidator, integerValidator]" :disabled="!(dataGConf.delayAccMode >= 1)" />
