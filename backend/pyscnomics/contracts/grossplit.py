...
class GrossSplit(BaseProject):
    ...
    _oil_ctr_share_before_transfer: np.ndarray = field(
        default=None, init=False, repr=False
    )
    _gas_ctr_share_before_transfer: np.ndarray = field(
        default=None, init=False, repr=False
    )
    ...
    _oil_ctr_share_after_transfer: np.ndarray = field(
        default=None, init=False, repr=False
    )
    _gas_ctr_share_after_transfer: np.ndarray = field(
        default=None, init=False, repr=False
    )
    ...
    def _wrapper_variable_split(
        self, regime: GrossSplitRegime = GrossSplitRegime.PERMEN_ESDM_20_2019
    ):
        ...
        if regime == GrossSplitRegime.PERMEN_ESDM_8_2017:
            ...
            or GrossSplitRegime.PERMEN_ESDM_20_2019
            or GrossSplitRegime.PERMEN_ESDM_12_2020
        ):
    ...
    def _get_prog_split_52_2017(fluid: FluidType, price: float, cum: float):
        ...
        if fluid == FluidType.OIL:
            ps = np.where(price > 0, (85 - price) * 0.25 / 100, 0)
    ...
    def run(
        ...
        prod_gas_boe = (
            self._gas_lifting.get_prod_rate_arr() / self.conversion_boe_to_scf
        )

        # Condition when the offset cumulative production array when user input is float
        if isinstance(cum_production_split_offset, float) or isinstance(
            cum_production_split_offset, int
        ):
        ...
        self._cumulative_prod = np.cumsum(
            self._oil_lifting.get_lifting_rate_arr() + prod_gas_boe + offset_arr
        )
        ...
        self._oil_carward_cost_aftertf = (
            self._oil_carward_deduct_cost - self._transfer_to_gas
        )
        self._gas_carward_cost_aftertf = (
            self._gas_carward_deduct_cost - self._transfer_to_oil
        )
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, GrossSplit):
            return all(
                (
                    np.allclose(
                        self._oil_lifting.lifting_rate, other._oil_lifting.lifting_rate
                    ),
                    np.allclose(
                        self._gas_lifting.lifting_rate, other._gas_lifting.lifting_rate
                    ),
                    np.allclose(self._oil_revenue, other._oil_revenue),
                    np.allclose(self._gas_revenue, other._gas_revenue),
                    np.allclose(
                        self._oil_tangible_expenditures,
                        other._oil_tangible_expenditures,
                    ),
                    np.allclose(
                        self._gas_tangible_expenditures,
                        other._gas_tangible_expenditures,
                    ),
                    np.allclose(
                        self._oil_intangible_expenditures,
                        other._oil_intangible_expenditures,
                    ),
                    np.allclose(
                        self._gas_intangible_expenditures,
                        other._gas_intangible_expenditures,
                    ),
                    np.allclose(
                        self._oil_opex_expenditures, other._oil_opex_expenditures
                    ),
                    np.allclose(
                        self._gas_opex_expenditures, other._gas_opex_expenditures
                    ),
                    np.allclose(
                        self._oil_asr_expenditures, other._oil_asr_expenditures
                    ),
                    np.allclose(
                        self._gas_asr_expenditures, other._gas_asr_expenditures
                    ),
                )
            )
    ...
    def __lt__(self, other):
        if isinstance(other, GrossSplit):
            ...
            other_total_base_cashflow = (
                other._oil_base_cashflow + other._gas_base_cashflow
            )
