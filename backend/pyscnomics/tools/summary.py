...
def get_summary(
    ...
    if np.sum(contract._oil_lifting.get_lifting_rate_arr()) == 0:
        ...
        oil_wap = np.divide(
            ...
            np.sum(contract._oil_lifting.get_lifting_rate_arr()),
    ...
    if np.sum(contract._gas_lifting.get_lifting_rate_arr()) == 0:
        ...
        gas_wap = np.divide(
            np.sum(
                contract._gas_wap_price * contract._gas_lifting.get_lifting_rate_arr()
            ),
            np.sum(contract._gas_lifting.get_lifting_rate_arr()),
            where=np.sum(contract._gas_lifting.get_lifting_rate_arr()) != 0,
        )
    ...
    sunk_cost_extended = np.concatenate(
        (
            ...
            np.zeros(
                len(contract.project_years) - len(contract._consolidated_sunk_cost)
            ),
        )
    )
    ...
    opex = np.sum(
        contract._oil_opex_expenditures + contract._gas_opex_expenditures, dtype=float
    )
    asr = np.sum(
        contract._oil_asr_expenditures + contract._gas_asr_expenditures, dtype=float
    )
